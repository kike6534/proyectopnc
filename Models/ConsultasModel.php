...
GROUP BY me.enfermedad_vigente";
		...
		public function selectNumeroTelAgentes()
		{
			...
			return $request;
				...
				public function selectAgentesConEspecialidad()
		{
            
			$sql = "SELECT op.num_oni,r.rango_policia, dp.nombre,dp.apellido,es.especialidad   FROM tbl_datos_personales dp 
INNER JOIN tbl_movi_especialidad me on dp.dui_pk=me.fk_dui_policia
INNER JOIN tbl_especialidades es on es.id_especialidad=me.fk_id_especialidad
INNER JOIN tbl_oni_policial op on op.fk_dui_policial=dp.dui_pk 
INNER JOIN tbl_asig_rango asigr on asigr.fk_oni_policial=op.num_oni
INNER JOIN  tbl_rango r on r.id_rango=asigr.fk_rango where op.estado ='1'";
			$request = $this->select_all($sql);
			return $request;
		}
